...
class KnowledgeGraph:
	...
	def add(self, prompt: str, response: str, judgment: str, regret_score: int, emotion: str, timestamp: Optional[str] = None) -> int:
		"""Add a new node to the graph."""
		node_id = len(self.graph.nodes) + 1
		self.graph.add_node(node_id, prompt=prompt, response=response, judgment=judgment, regret_score=regret_score, emotion=emotion, timestamp=timestamp or datetime.now().isoformat())
		if node_id > 1:
	...
	def visualize(self, out_path: str = 'graphs/graph.png') -> None:
		...
		node_colors = ['red' if self.graph.nodes[n].get('regret_score', 0) > 7 else 'lightblue' for n in self.graph.nodes]
		labels = {n: self.graph.nodes[n]['prompt'][:20] + ('...' if len(self.graph.nodes[n]['prompt']) > 20 else '') for n in self.graph.nodes}
	...
	def causal_forgetting(self, regret_threshold: int = 3, age_days_threshold: int = 7) -> int:
		"""Prune nodes with low regret or old age."""
		...
		for node in list(self.graph.nodes):
			...
			if data['regret_score'] < regret_threshold and (age_days > age_days_threshold or connectivity < 1):
	...
	def check_past_regrets(self, prompt: str, regret_threshold: int = 7) -> Any:
		high_regret_nodes = [n for n in self.graph.nodes if self.graph.nodes[n].get('regret_score', 0) > regret_threshold]
